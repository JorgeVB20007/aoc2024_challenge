...
int main()
{
	...
	int current_rock = 1022;

	Tile *end = NULL;
	Tile *map = create_map(input, &end, 71, current_rock);

	do
	{
		reset_map(map);
		access(map, input[current_rock].first, input[current_rock].second)->corrupted = true;
		explore(map, end);
		current_rock++;
	}
	while (end->coming_from);

	std::cout << ">>> " << input[current_rock - 1].first << "," << input[current_rock - 1].second << std::endl;
	delete(map);
}