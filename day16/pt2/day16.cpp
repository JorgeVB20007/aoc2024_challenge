...
const Visit &Tile::best_score_prev() const
{
	int best = -1;
	int best_dir = 0;
	for (int dir = 0; dir < 4; dir++)
...
int Tile::best_score_coming_from(int coming_from) const
{
	int best = -1;
	for (int dir = 0; dir < 4; dir++)
	{
		int attempt = visits[dir].score + (spin_cost(coming_from, dir) == 1) * 1000;
		if (visits[dir] && (best == -1 || attempt < best))
			best = attempt;
	}
	return (best);
...
void fck_go_back(Tile *loc, Tile *end, int coming_from)
{
	...
	for (int dir = 0; dir < 4; dir++)
	{
		...
		{
			if (dir == NORTH)
				fck_go_back(loc->south, end, NORTH);
			else if (dir == EAST)
				fck_go_back(loc->west, end, EAST);
			else if (dir == SOUTH)
